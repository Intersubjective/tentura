...
Future<void> _awaitFetchCount(FakeBeaconThreadsRepository room, int expected) async {
  final deadline = DateTime.now().add(const Duration(seconds: 2));
  while (DateTime.now().isBefore(deadline)) {
...
void main() {
  ...
  group('RoomCubit.sendMessage reply quote', () {
    ...
    test(
      ...
      () async {
        ...
        final gate = Completer<void>();
        final fakeRoom =
            FakeBeaconThreadsRepository(userId: kRoomCubitFakeMyUserId)
              ..messages = [target]
              ..createMessageGate = gate;
        final cubit = roomCubitForTest(fakeRoom);
        addTearDown(cubit.close);
  ...
  group('RoomCubit.jumpToRepliedMessage', () {
    ...
    test(
      ...
      () async {
        registerRoomCubitProfileCubit(kRoomCubitFakeMyUserId);
        final fakeRoom = FakeBeaconThreadsRepository(userId: '')
          ..messages = [
            _roomMsg(
              id: 'm1',
              createdAt: _kBaseTime,
