...
class WebSocket {
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      // Tentura workaround (beyond PR #87) — remove with packages/web_socket_client
      // override when upstream also handles channel errors:
      // https://github.com/felangel/web_socket_client/pull/87
      // Without onError, WebSocketChannelException escapes as an unhandled
      // async error (Sentry fatal) while cancelOnError tears down.
      _subscription = _channel!.stream.listen(
        (message) {
          if (_messageController.isClosed) return;
          _messageController.add(message);
        },
        onError: attemptToReconnect,
        onDone: attemptToReconnect,
        cancelOnError: true,
      );
    } on Object catch (error, stackTrace) {
      // Tentura workaround — pair with Completer guards in
      // https://github.com/felangel/web_socket_client/pull/87:
      // StateError from Completer races is an Error, not Exception —
      // `on Exception` let it escape unhandled.
