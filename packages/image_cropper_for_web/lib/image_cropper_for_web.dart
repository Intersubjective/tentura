...
class ImageCropperPlugin extends ImageCropperPlatform {
  ...
  /// Cropper.js [Cropper.scale] updates `imageData.scaleX` / CSS scale, but
  /// [Cropper.getData] maps the crop box using `imageData.width / naturalWidth`
  /// and ignores that transform, so [Cropper.getCroppedCanvas] exports look
  /// ~2× zoomed vs the on-screen preview. The stock slider is wired to
  /// `scale`; use [Cropper.zoomTo] so preview and export stay consistent.
  static double? _webSliderZoomBaseRatio;
  ...
  Future<CroppedFile?> cropImage({
    ...
    void doScale(num value) {
      if (cropper == null) throw 'cropper has not been initialized';
      final canvas = cropper!.getCanvasData();
      final nw = canvas.naturalWidth;
      if (nw == 0) {
        return;
      }
      _webSliderZoomBaseRatio ??= canvas.width / nw;
