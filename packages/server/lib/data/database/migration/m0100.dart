...
/// Active forward-edge dedup: one live edge per (beacon, sender, recipient).
final m0100 = Migration('0100', [
  '''
UPDATE public.beacon_forward_edge e
SET cancelled_at = now()
WHERE e.cancelled_at IS NULL
  AND EXISTS (
    SELECT 1 FROM public.beacon_forward_edge k
    WHERE k.cancelled_at IS NULL
      AND k.beacon_id = e.beacon_id
      AND k.sender_id = e.sender_id
      AND k.recipient_id = e.recipient_id
      AND (k.created_at < e.created_at
           OR (k.created_at = e.created_at AND k.id < e.id))
  );
''',
  '''
CREATE UNIQUE INDEX IF NOT EXISTS bfe_active_unique
  ON public.beacon_forward_edge (beacon_id, sender_id, recipient_id)
  WHERE cancelled_at IS NULL;
''',
  // Exclude cancelled edges from inbox provenance (companion to dedup cleanup).
  r'''
