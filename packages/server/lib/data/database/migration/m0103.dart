...
/// Inbox forward provenance: exclude self as forwarder; count invite-forward
/// edges (NULL context) even when inbox_row.context is set.
final m0103 = Migration('0103', [
  r'''
...
BEGIN
  ...
  WITH senders AS (
    ...
    WHERE bfe.recipient_id = inbox_row.user_id
      ...
      AND bfe.sender_id <> inbox_row.user_id
      AND bfe.recipient_rejected = false
      AND bfe.cancelled_at IS NULL
      AND (
        nullif(trim(inbox_row.context), '') IS NULL
