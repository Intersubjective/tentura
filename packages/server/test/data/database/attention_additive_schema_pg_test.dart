...
'''),
    ...
    // This replaced a test that replayed every m0178 statement twice to prove
    // restart safety. The statements now live inside the squashed baseline,
    // which is not idempotent by construction (`pg_dump` emits bare CREATEs),
    // and migrant applies it once. What is still worth asserting is that the
    // schema carries each constraint once and all five columns.
    test('carries the clear/task columns and no duplicate constraints',
        () async {
