...
**Withdraw (`beaconWithdraw`)** — user-initiated withdraw is allowed only while the beacon is in the **open family** (including `enoughHelp` / `needsMoreHelp`). Once the author has **Closed** and the beacon enters **Wrapping up** (`reviewOpen`), withdraw is **forbidden** (`beaconWithdrawForbidden`); participation is already closed by the author's close, and reviewers may skip instead.

While the beacon is still open-family, withdraw records an append-only commitment event and typically places the helper in **Watching** (see watching doc). When the beacon is **not** in the open family, the server tombstones the inbox row instead of leaving a triage-style **Watching** row — that path is **not** reachable via ordinary user withdraw after P3.8; it remains for **block-driven cleanup** and other server-side writers.
