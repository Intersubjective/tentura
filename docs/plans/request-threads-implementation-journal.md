...
- 2026-08-14 — UNIT 13: `899b3cbe4` glossary — terminology.mdc, AGENTS.md, CONTEXT.md
  (discussion workspace; thread/General outside table).

- 2026-08-14 — UNIT 13: `a28c70006` ARB three-sense sweep (~60 EN keys); delete
  `labelBeaconTabRoom`/`labelBeaconTabItems`/`coordinationItemDiscussionTitle`; reword
  discussion→thread keys; Dart call sites (`threadTitleFallback`, `labelBeaconTabThreads`).

- 2026-08-14 — UNIT 13: `56163038b` server notification/error copy (discussion vs thread
  senses); message mutation test name alignment.

- 2026-08-14 — UNIT 13: `0eeedc870` normative `beacon_room.md` Threads spec; README
  pointers; landing onboarding discussion copy (preserved availability plan row).

- 2026-08-14 — UNIT 13: `04b3b24bd` client `6.1.1` + web cache-buster; `kDefaultMinClientVersion`
  unchanged.

- 2026-08-14 — **UNIT 13 complete.** Verify: server scoped tests (75 passed across four files);
  EN jq Chat matcher empty; RU jq allowlist =
  ...
  docs/landing/server-notification `rg` gates clean; `notificationCatUnblocksMe` survivors intact;
  `6.1.1` pubspec + cache-buster; `check-user-facing-terminology.sh` ok; custom lints client 93/93
  server 0/0; full `flutter test` (exit 0); server `dart test --exclude-tags pg` (1509 passed).
