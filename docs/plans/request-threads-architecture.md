...
> **Superseded UX (shipped 2026-09).** Request detail no longer uses a **Threads \| People \| Log** tab bar or a pushed `ThreadDetailRoute`. The shipped surfaces are **NOW / Chat / People** (Activity log is an overflow sheet). General is inline on the **Chat** tab (compact/regular) or the permanent right pane (expanded split). See [`request-detail-now-chat-people-plan.md`](request-detail-now-chat-people-plan.md) and [`docs/features/beacon_room.md`](../features/beacon_room.md). The body below documents the intermediate threads-list architecture this branch explored; keep it for server/list-contract history, not current navigation.
...
| # | Decision |
|---|---|
| **D1** | **Threads replaces Items as tab 0.** Tab bar stays three-wide: `Threads \| People \| Log`. → **Shipped as NOW / Chat / People** (no threads list tab; General is on Chat / split pane). |
| **D2** | **General is row 0 of the Threads list** — for viewers who have room access (§4.4). → **Shipped:** General is the sole conversation on Chat / split pane (no list row). |
...
| **D6** | **Expanded split:** left = header/HUD + tab bar + tab body; right = the selected thread, **General preselected**, staying co-visible when the left switches to People or Log. → **Shipped:** left = NOW + People only; right = General permanently; Activity log is a sheet. |
| **D7** | **The app-bar chat button and the `?tab=room` / `?surface=room` surface are deleted**, and their producers rewritten (§9.3). General is reachable only through the Threads list. → **Shipped:** General via Chat tab or split pane; `?tab=threads` wire value preserved. |
...
| **D12** | **Threads tab badge = General unread + thread unread**, with the closed-thread question settled in §5.4. → **Shipped:** Chat tab badge = General unread only (`threadsTabUnreadCount`). |
...
| **D24** | **Thread detail on compact/regular is a real route**, not a query-param variant of the current page (§8.3). → **Retired:** `ThreadDetailRoute` / `ThreadDetailScreen` deleted; General is an inline `BeaconRoomSurface` on the Chat tab. Legacy `/thread/:id` URLs rewrite to `?tab=threads&thread=…`. |
...
1. **Nested child route** — `ThreadDetailRoute` registered as a *child* of the beacon-view route, so
   ...
   → **Not shipped.** U10 retired the pushed route; see the supersession note at the top.
...
```
/beacon/view/:id?tab=threads                 → list (base page) — historical; shipped UX uses inline Chat
/beacon/view/:id/thread/:threadId            → ThreadDetailRoute (distinct page, nested child) — **retired**; rewrites to query form
```
...
- **Settled (shipped):** request-detail tabs are **NOW / Chat / People** (`labelBeaconTabNow`, `labelBeaconTabChat`, `labelBeaconTabPeople`). General's title is **General / «Общее»**. Activity log is an overflow sheet (`labelBeaconTabLog`), not a tab.
- **Historical (this doc):** the tab was **Threads / Темы**, and General's title is **General / «Общее»** (owner
