...
**Before U3 (UX10, U0):** reconcile that statement with `../../CONTEXT.md` § Beacon visibility & sharing, which still describes the older forwarding/participation-based access rule and says MeritRank is never a visibility gate. Inspect the live permission functions, migrations, and visibility tests; record separately (a) implemented behavior with evidence, (b) the accepted target in D4/D11/D14, and (c) the migration/rollout condition that activates it. Annotate the status-quo discovery paragraph as intended behavior until verified active. Update `CONTEXT.md` to distinguish its current contract from the target instead of silently claiming the new rule already applies. Direct trust, discoverability, forwarding, and discussion admission must each have an unambiguous meaning across all three documents. U0 is a documentation/evidence gate, not permission to bypass §14.1.
