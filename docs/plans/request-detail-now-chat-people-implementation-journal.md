...
### [U11] complete — 2026-09-08T00:45:00+02:00
COMMITS: 87652aedc docs: ship NOW/Chat/People surfaces in product and agent docs; ebd733627 chore(client): bump to 7.1.0 for request-detail navigation change / TESTS: `bash scripts/check-doc-drift.sh` → exit 0; `bash scripts/check-user-facing-terminology.sh` → exit 0; `cd packages/client && flutter analyze --no-fatal-warnings --no-fatal-infos` → exit 0, **0 errors, 89 warnings**; `cd packages/client && flutter test` → **+2725 passed, ~30 skipped, 0 failed** / FILES: `docs/features/beacon_room.md`, `docs/client-ui-inventory.md`, `docs/plans/request-threads-architecture.md` (tracked for first time with supersession banner), `docs/tentura-design-system.md`, `.cursor/rules/terminology.mdc`, `CONTEXT.md`, `packages/client/pubspec.yaml`, `packages/client/web/index.html`, journal / FINDINGS: (1) **`docs/Tentura_current_status_quo.md` DEFERRED** — still dirty-uncommitted (concurrent editor per journal blockers); plan §9 lines **125** (`Discussion / People / Log` → `NOW / Chat / People`), **127** (discussion paragraph), **204** (summary line) remain for the owner. (2) **`docs/plans/constellation-edge-semantics.md` untouched** (hands-off). (3) Web cache-buster edited by hand (`?v=7.1.0`) — no `flutter build web` run this session. (4) `kDefaultMinClientVersion` unchanged (client-only UI). (5) Code wins over plan where they differ: split latches on `_hadThreadRowsAtLeastOnce`; narrowing from split always selects Chat (not preserving left-pane surface); People tab is icon-only at `tabCompactWidth` with badge overlay precedence. / REMAINING: U12 new test suites T1–T10; owner applies deferred `Tentura_current_status_quo.md` edits
