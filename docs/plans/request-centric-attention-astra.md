...
7. Generalize answered-forward dismissal and implement complete For You sweeping.
...
A “You’re helping” outcome therefore belongs on the My Desk Request card after offering help. For You may show a transient navigation snackbar, not a durable duplicate Request row. A legacy helping row must remain dismissible during migration.
...
### D07 — For You has a complete, explicit sweep operation

**Dismiss all** covers the entire server-authorized For You attention set, including unloaded pages:

| Item at capture | Sweep effect |
|---|---|
| Request optional events | Clear captured events |
| Answered-forward outcome | Clear captured outcome generation |
| Unanswered forward | Apply the existing explicit “not interested” transition and clear the resulting outcome in the same per-Request transaction |
| Pending invite setup prompt | Apply the existing Skip transition and clear its update |
| Network optional receipt | Clear receipt |
| Live obligation / Request now owned by My Desk | Skip; never dismiss |

The button’s accessible description states that unanswered forwarded Requests will move to Not interested and pending setup prompts will be skipped. No confirmation modal is necessary for this reversible action.

A single forward card’s existing Reject action may leave a dismissible “Not interested · Restore” outcome. **Dismiss all must not generate a fresh uncleared outcome of its own.**

Watching remains a collection of Inbox stances, not a list of uncleared events. Sweeping a Watching update does not stop watching.

**Rationale/evidence:** existing Activity Read all cannot remove outcome rows or skip prompts; the owner explicitly requires a surface capable of reaching zero (`C/features/inbox/ui/screen/inbox_screen.dart:261–283`; `docs/plans/work-activity-redesign-plan.md:154–187`).
...
- The minimum client version prevents old clients from continuing generic settlement or presenting misleading indicators after activation.
...
- Every outcome and optional event has `×`.
...
- Clean state replaces the attention stream when it reaches zero.
...
| **U18 Backfill and compatibility activation** | U05–U17 | Restartable migration/backfill tooling, capability/min-version gate | Fixed cutover, no notification replay, legacy cleanup, old-client behavior explicitly blocked |
...
| **D07** | Sweep across multiple unloaded pages and every item kind; pending forward goes to Not interested without residual outcome; prompt is skipped; Watching stance survives; responsibility gained during sweep causes skip |
...
These are bounded product choices; the defaults above make the implementation handoff concrete.
...
| Does Dismiss all explicitly reject unanswered forwards and skip pending prompts? | **Yes**, with clear button semantics and Undo. Otherwise the requested zero state remains impossible. |
| How should “You’re helping” coexist with the one-primary-surface rule? | Put the dismissible outcome on the My Desk Request card; use only transient feedback in For You. |
...
| Should launch include a daily clearing number? | Illustration and exact per-operation cleared count are mandatory; daily count can ship with them only if timezone and undo accounting pass the specified tests. |

The owner’s stated requirements—dismissible terminal outcomes, complete For You sweeping, no passive clearing, obligation-only promotion, and Request-level ownership—are fixed requirements, not open questions.