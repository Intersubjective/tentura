...
All 15 Fable comments were checked against live source and D1–D37. Corrections below are applied; this is not implementation/test acceptance. “Adjusted” means the issue is useful but the proposed fix or its premise needed correction.

| ID | Result | Evidence and resolution |
|---|---|---|
| R1 | Accepted | `query_constellation_field.dart` has no context argument and supplies `kConstellationContext`. Keep context server-owned in C4/P04. |
| R2 | Accepted | `domain/exception.dart` serializes `extensions.code` as a decimal string. C4/P04 use the existing numeric error model, not symbolic wire codes. One typed exception class suffices; four subclasses are unnecessary. |
| R3 | Adjusted | Server realtime contract tests enumerate publisher migrations and specialized function names. Name both triggers and register publisher/manifest/tests together in P08 when subscriber/evidence files exist. Pass `lower(TG_OP)` to the strict publisher: raw PostgreSQL `TG_OP` is uppercase and would be rejected. |
| R4 | Accepted | `websocket_path_entity_changes.dart` drops all actor sessions with echo disabled. Exempt only the private anchor kind; test both flag values. |
| R5 | Adjusted | Viewer cascades can remove the cursor first; target cascades can invert row/cursor lock order. Define missing-cursor behavior and one retry of a definitely aborted transaction in the data adapter, not SQLSTATE handling in domain. Do not replay an ambiguous client timeout. |
| R6 | Accepted | Verified on local PostgreSQL 17.9: `NaN = NaN` is true; NaN and both infinities each fail `BETWEEN -10 AND 10`. Specify the exact CHECK and PG fixtures. |
| R7 | Accepted | Current `UserEntity.newId` / `BeaconEntity.newId` use `U` / `B`; existing field-node IDs already work. Keep raw graph IDs and typed anchor keys; remove the graph re-keying work. Prefixes are not an authorization or wire-type check. |
| R8 | Rejected | Revision orders mutations including deletes; `placedAt` implements the separately accepted D22 paint-order contract. Revision-only paint order would change D22. Keep server-monotonic placement time and the specified deterministic tie-break. |
| R9 | Adjusted | `TenturaDb` has no read-snapshot helper; profile lookups already share its Drift transaction zone. Add the explicit helper. Further tracing found `beacon_can_read_content` calls a cache-writing function from m0163a: add a read-only cache-miss path and cold-cache tests, otherwise the suggested transaction fails. |
| R10 | Adjusted | `_PersonRequestGroup` actually renders only `visibleIds`, not every Request. Nevertheless Text-first/unlaid-target pinning needs a defined fallback. Compute one unanchored candidate without changing other positions; do not temporarily pretend the target is already pinned. |
| R11 | Adjusted | `edges_view.dart` already repaints from the controller and reads its layout. Add same-frame edge regression coverage and inspect both composition files; require edits only if the new hook needs them. |
| R12 | Accepted | Validate kind, non-empty ID and non-ego person before either mutation. Delete idempotence applies only to a valid target key. |
| R13 | Adjusted | Reuse `BeaconStatus.fromSmallint` for the presenter, but first validate known renderable wire values: its unknown-value fallback is `open`, which would violate C2. |
| R14 | Accepted | C2 excludes wrapping-up status 5 by default. D5/D36 now state when the node disappears and when its review marker is visible. |
| R15 | Adjusted | The single-app runner is insufficient, but `run_realtime_multiclient_web_local.sh` and `test_driver/realtime_multiclient_web_test.dart` already create independent browser sessions. Use that infrastructure; API-origin remote writes are supplementary coverage, not two-browser acceptance. |

Evidence paths are specified in the affected packets. Only the plan was edited. Read-only PostgreSQL checks confirmed R6 scalar behavior and `SET TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY` (`repeatable read|on`); feature migrations/runtime were not implemented or tested.
...
When a pinned Request enters wrapping up or closes, its node disappears from
the default Constellation view while its anchor remains dormant. If the Request becomes eligible for the
...
| Wrapping up / review (Show closed only) | informational | time marker |
...
- `ConstellationAnchorTarget` is a sealed `person(id)` / `beacon(id)` value. Its wire kind is `PERSON` / `BEACON`. IDs are existing opaque text IDs, not assumed UUIDs. Never key anchor maps by a naked target ID: use the typed target; retain existing raw IDs for `FieldPersonNode` / `FieldRequestNode` and graph lookups. Map a typed target to its existing node ID at the adapter boundary; do not infer target kind or authorization from an ID prefix. No graph re-keying or `node_details.dart` edit is required.
...
| Unpin | First validate kind, non-empty opaque ID and non-ego person, as for upsert. For a valid target key delete only the viewer-owned anchor; return the same idempotent success shape whether the target/anchor exists or is readable |
...
Validate the raw status against `{0,7,8,5,4,6}` before presentation (and apply the selected lifecycle filter); reject malformed/unknown values at the data boundary. Convert valid values with shared `BeaconStatus.fromSmallint`, then switch exhaustively on `BeaconStatus` in the presenter. Do not duplicate its integer decoder: its unknown-value fallback is `open` and cannot be used as validation. The SQL/wire-to-label reference mapping is exact: `0 → Open / open-circle`, `7 → Needs more help / add-help`, `8 → Enough help / check`, `5 → Wrapping up / review / time`, `4,6 → Closed / completed`. Concrete Material icons: `circle_outlined`, `person_add_alt_1_outlined`, `check`, `schedule`, `task_alt`, respectively; pin uses `push_pin`. Semantic tones, in the same order: `tt.border`, `tt.warn`, `tt.good`, `tt.info`, `tt.textMuted`. Use localized state text and a separate pin glyph. Viewer involvement remains label/preview content.
...
Partial unique indexes: `(viewer_id,person_id) WHERE person_id IS NOT NULL` and `(viewer_id,beacon_id) WHERE beacon_id IS NOT NULL`. Add target-side indexes for FK cascades. Use exactly `CHECK (coordinate_space_version = 1 AND x_units BETWEEN -10 AND 10 AND y_units BETWEEN -10 AND 10)` with the declared NOT NULL columns. PostgreSQL orders NaN above finite values, so these bounds reject NaN and both infinities; do not add an ineffective `x_units = x_units` guard. The verified SQL account table is `public."user"` (Drift `Users`); use its existing text ID type, not a new Person table.
...
2. Use UPDATE-if-present / INSERT-if-absent under that lock, not an unchecked read followed by `ON CONFLICT` revision allocation. A BEFORE row trigger `bump_constellation_anchor_revision()` allocates revision for INSERT/UPDATE/DELETE using `UPDATE constellation_anchor_cursor SET revision = revision + 1 WHERE viewer_id = <row viewer> RETURNING revision`. Missing cursor on INSERT/UPDATE is an invariant error (repository must create it first). On DELETE with no cursor, return OLD without allocation or publication: this covers viewer-account cascade order. For INSERT/UPDATE set `NEW.revision` and `NEW.placed_at = greatest(clock_timestamp(), last_placed_at + 1 microsecond)`; update `last_placed_at`. Clients cannot write either field.
...
4. An AFTER row trigger `notify_constellation_anchor_change()` publishes the C8 invalidation after allocation; on DELETE it skips publication if the viewer cursor or account is absent. FK target deletion follows these same triggers. Never recreate a deleted account's cursor. Rollback rolls back cursor, row and notification together.
...
No persistent delete tombstones are needed: retain the viewer cursor after unpin, and return a complete authorized anchor projection with its watermark. Target cascades can lock anchor rows before the cursor, opposite to upsert; a deadlock is possible. In the anchor data adapter retry the **whole** top-level transaction once only for PostgreSQL SQLSTATE `40P01` / `40001`, after the failed transaction has fully rolled back. Re-run locking and authorization; never retry inside the failed transaction or leak SQLSTATE handling into domain. A second failure follows normal command failure/rollback. A cascade initiated by another mutation may itself be the deadlock victim: preserve that caller's atomic rollback/error handling; do not claim both concurrent commands must always succeed or add an unrelated global retry policy. Test both victim directions. Never automatically replay a client write after an ambiguous network timeout.

### C4 — One field API, two read projections

Extend the existing field as `constellationField(showClosed: Boolean = false, participatedOnly: Boolean = false, projection: ConstellationProjection = FULL)`, where the enum values are `FULL | ANCHORS`. Do not expose a `context` argument: the resolver supplies existing `kConstellationContext` for reads and writes; response context is server-authored. The repository's `context` parameter remains internal. Add `constellationAnchorUpsert(targetKind, targetId, xUnits, yUnits, coordinateSpaceVersion)` and `constellationAnchorDelete(targetKind, targetId)`. JWT supplies viewer ID; no viewer input. Mutations use the same server-owned `kConstellationContext` for person authorization.
...
FULL reads automatic selection, authorized anchors, pinned targets, support and cursor in **one read-only REPEATABLE READ transaction** via `ConstellationFieldRepositoryPort.readSnapshot(viewerId, context, filters, projection)`, returning `ConstellationFieldSnapshot`. Keep constituent query helpers private in the adapter. Add `TenturaDb.withReadSnapshot<T>(Future<T> Function() action)` using an awaited Drift `transaction(() async { await customStatement('SET TRANSACTION ISOLATION LEVEL REPEATABLE READ READ ONLY'); return action(); })`; SET must be the first statement. Use it as the top-level read boundary, never nested inside an existing mutation/transaction. `_profiles` already uses the same `TenturaDb`; calling it inside this Drift transaction zone joins the transaction without rebinding or a new repository. Do not launch detached work outside that zone. No new external network reads inside this transaction; the guarantee is one PostgreSQL MVCC snapshot, not transactionality of the existing external MeritRank engine. If no cursor exists, read watermark `0` without creating a row in this read-only transaction.

Make the existing read wall compatible first: `beacon_can_read_content` (m0162) calls `person_are_mutually_visible_cached` (m0163a), which INSERTs on a cold/stale cache. In the new migration replace that cache function preserving ordinary behavior; after its first valid-cache lookup misses and **before** advisory locking, check `current_setting('transaction_read_only') = 'on'`. In that branch compute `person_are_mutually_visible` with the existing fail-closed exception handling and return it without advisory lock or cache write. The read-write branch stays unchanged. Do not bypass `beacon_can_read_content`, prewarm as a correctness requirement, swallow read-only errors as absent targets, or weaken authorization.
...
Follow `ExceptionBase` and numeric `extensions.code` strings. In `S/domain/exception_codes.dart` add `ConstellationExceptionCode { invalidTarget, invalidCoordinates, unsupportedCoordinateSpace, targetUnavailable }` in that immutable order and `ConstellationExceptionCodes` with currently unused `codeSpace = 1700`. Recheck uniqueness at implementation time; update this contract and fixtures together if another change has claimed the space. Add one `ConstellationException extends ExceptionBase` accepting this enum in `S/domain/exception.dart`.

Wire mapping: `invalidTarget → "1700"`, `invalidCoordinates → "1701"`, `unsupportedCoordinateSpace → "1702"`, `targetUnavailable → "1703"`. Missing and unauthorized upsert targets share `"1703"` with the same generic description and no target details. Client data mapper converts numeric strings into a typed feature-domain failure; domain/UI never inspect GraphQL maps. Keep existing authentication/version errors and normal GraphQL coercion errors for malformed enum/scalar inputs. Upsert response contains authoritative anchor and `revision`; delete contains target and `revision`.
...
- Map and Text derive from one composed presentation result. Text keeps its current non-spatial grouping and existing overflow controls; eligible pinned items are included even when normal satellite budgets are exhausted.
...
Use a load-generation token for every filter/account change; stale responses cannot overwrite newer state. Ignore older projection revisions. Equal revisions may still refresh authorization/membership (permissions can change without anchor writes); only the latest matching request generation may apply them. Subscribe before initial FULL load; coalesce hints to one in-flight ANCHORS read plus one queued rerun. Reconnect requests ANCHORS once. Remote echoes do not show snackbars. Reconciliation never changes the field timestamp unless a matching FULL result is accepted.

### C8 — Private invalidation and rollout

Add realtime kind `constellation_anchor`, aggregate ID = viewer account ID, payload event `insert|update|delete`; no target, position, title or revision payload is necessary. Recipient list is exactly `[viewer_id]` and is stripped before client delivery. Use existing bounded `entity_changes` publisher/fan-out, not Hasura subscriptions or polling. Anchor invalidations always reach all sessions of the owning account, independently of the compatibility echo flag. Add `kRealtimeAlwaysEchoKinds = {'constellation_anchor'}` in `S/consts/realtime_consts.dart`; in `websocket_path_entity_changes.dart` skip actor recipients only when echo is disabled **and** the entity is not in this set. Retain recipient validation/stripping and every other kind's current policy.
...
P02 defines `notify_constellation_anchor_change()` as an AFTER INSERT/UPDATE/DELETE row trigger. After C3's missing-cursor/account check, call `emit_realtime_entity_change_strict('constellation_anchor', viewer_id, lower(TG_OP), ARRAY[viewer_id])`. Uppercase `TG_OP` is invalid for this publisher. For an absent-key unpin the repository increments the cursor once and explicitly calls this strict publisher with event `'delete'` in the same transaction; no row trigger fires. Never swallow publisher failure or emit a second notification for an existing-row delete.

Manifest entry: `wireKind: constellation_anchor`, `acceptedWireKinds: [constellation_anchor]`, `clientKind: constellationAnchor`, `genericTriggerArgs: []`, `specializedPublishers: [notify_constellation_anchor_change]`, `impacts: [constellation_anchors]`; its `tests` name the new anchor case/cubit tests. In P08, together with the actual subscriber and its tests, update the hard-coded publisher migration list and expected specialized-publisher set in `packages/server/test/architecture/realtime_entity_contract_test.dart`, migration notification tests, client `fromWire`, and the `impactSubscribers` map in `packages/client/test/architecture/realtime_entity_contract_impacts_test.dart`.
...
Implement C3, cursor/row triggers, cascade handling, SQL indexes and typed repository results. Add a domain-owned transaction/snapshot operation instead of leaking `TenturaDb` into use cases. Implement the named BEFORE/AFTER triggers and strict publisher from C3/C8, with focused migration/notification tests. Add protocol/manifest registration and its architecture-test updates in P08, when the real subscriber/evidence files exist; do not create placeholder tests/subscribers to make P02 green. Include the C4 read-only cache-miss branch in this migration. Regenerate Drift/DI.

**Check:** disposable PostgreSQL tests for all CHECK/FK/unique constraints (explicit `'NaN'::float8`, `'Infinity'::float8`, `'-Infinity'::float8` for each coordinate), owner isolation, same-target upsert race, different-target ordering, delete-vs-move, absent delete, monotonic placement time, rollback with no notification, both target cascades, both viewer/cursor deletion orders, first-attempt deadlock/serialization rollback plus successful one-retry outcome, retry exhaustion with no partial commit, and both deadlock victim directions. Assert exactly one lowercase owner-only notification per committed command and none on abort; an absent-key unpin also emits one. Run fresh migration and upgrade from pre-feature schema. Migrant is forward-only here: document coordinated application rollback retaining additive tables; do not invent/destructively run a down migration.
...
Implement C2/C4/C5, including batched participation, authorized dormant anchors, pinned-only authors, bounded-hop closure outside automatic caps, FULL/ANCHORS and consistent transaction watermark through the new C4 `withReadSnapshot` helper (owned `S/data/database/tentura_db.dart`). Share the canonical selection/projection builder between reads and mutation validation; do not bypass `beacon_can_read_content`.

**Check:** extend `test/domain/use_case/constellation_field_case_test.dart` and `test/data/repository/constellation_field_repository_pg_test.dart`; new participation/path parity tests. Fixtures: cap+1 automatic candidates plus pins/support; no active Request for pinned person; readable non-discoverable pinned Beacon; blocked/unblocked target retains coordinates; author outside normal graph; shared support; no three-hop path; every C2 status/participation branch. Assert excluded IDs/counts do not leak and ANCHORS does not query automatic discovery. From inside the actual snapshot assert `current_setting('transaction_isolation') = 'repeatable read'` and `transaction_read_only = 'on'`, including profile reads. Test cold/stale/warm visibility caches: readonly results match the canonical predicate, authorized discovery-only content remains readable, and readonly reads neither change cache rows nor need prewarming. Preserve/test ordinary read-write cache population.

### P04 — Authenticated V2 API

**Files:** new `S/domain/use_case/constellation_anchor_case.dart`, GraphQL `mutation/mutation_constellation_anchor.dart`; `query/query_constellation_field.dart`, `mappers/constellation_gql_maps.dart`, `custom_types.dart`, `query/_queries_all.dart`, `mutation/_mutations_all.dart`; `S/domain/exception_codes.dart`, `S/domain/exception.dart`; DI sources.

Expose exact C4 signatures with server-owned context, non-null lists, enum validation, numeric-string error codes and decimal revision strings. Use the actual GraphQL registry pattern; do not add another HTTP route. Authenticate before resolving any field/mutation.

**Check:** new `test/api/controllers/graphql/constellation_anchor_test.dart`: missing/forged viewer input, rejected public context argument, invalid kinds/coordinates/version, exact numeric-string domain codes versus ordinary GraphQL coercion failures, missing vs forbidden indistinguishable error, empty-ID/ego delete rejection, valid-target idempotent delete, authoritative mutation results and projection mode defaults. Run API tests against the emitted schema, not mapper mocks alone.

### P05 — Client wire adapters and server echo policy

**Files:** `F/data/gql/constellation_field_fetch.graphql`; new `constellation_anchors_fetch.graphql`, `constellation_anchor_upsert.graphql`, `constellation_anchor_delete.graphql`; `F/data/repository/constellation_repository.dart`, `_mock.dart`; new `F/domain/port/constellation_anchor_repository_port.dart` and `F/data/repository/constellation_anchor_repository.dart`; new `F/domain/exception.dart` and `F/data/model/constellation_error_mapper.dart`; `C/data/gql/schema.graphql`, `C/data/service/remote_api_client/build_client.dart`; new `S/consts/realtime_consts.dart`, existing `S/api/controllers/websocket/path_handler/websocket_path_entity_changes.dart`, and its WS protocol tests.
...
**Check:** extend `constellation_repository_test.dart`; validate routing, enums, precision and mapping. Run the WS protocol tests for owner-only fan-out with two sessions of the actor and a foreign-account session, for both true and false `REALTIME_ACTOR_ECHO_ENABLED`; verify unrelated kinds retain flag behavior. Run existing transport coalescing tests and assert numeric error-to-domain mapping; anchor-specific catch-up is verified in P08. Leave the client realtime enum/manifest unchanged until P08 introduces the subscriber; wire-adapter tests do not require a registered subscription.

### P06 — Pure composition, budgets and layout

**Files:** `F/domain/constellation_cap_policy.dart`, `constellation_density.dart`, `constellation_layout.dart`, `constellation_consts.dart`, `constellation_filters.dart`, `domain/use_case/constellation_field_case.dart`; new `F/domain/constellation_anchor_composition.dart`, `F/domain/constellation_pin_position.dart`; `C/features/graph/ui/utils/tentura_layout_algorithms.dart`.
...
**Check:** existing Constellation cap/density/filter/path/layout tests plus new anchor composition/layout tests: person/Beacon independence, all pins survive budgets, topology changes preserve anchors, repeatability, collisions, exact overlapping anchors, exhausted candidates, hint eligibility, all four envelope corners and compact/expanded coordinate round-trip. Confirm Map/Text eligible-ID parity; add Text-first/density-excluded single-target fallback tests asserting no change to other positions or anchor state.

### P07 — Graph gesture adapter

**Files:** `G/graph_view.dart`, `configuration.dart`, `controller.dart`, `widget/nodes_view.dart`, `widget/labels_view.dart`; new `G/widget/node_drag_gesture.dart`; graph package public exports if required. Inspect `G/widget/edges_view.dart` and `G/widget/graph_layout_view.dart` as regression scope; edit only if the new hook requires it. `EdgesView` already listens to controller repaint and reads `controller.layout`.
...
**Check:** new graph-package widget tests: pan/zoom without capture; scale-before-capture; additional finger after capture; remaining finger after primary lift; cancel; topmost overlap; only one node/incident edge geometry changes during movement; both incident edge endpoints follow the updated controller position in the **same frame**, without needing widget rebuild or global relayout. Unrelated edge geometry stays identical (a whole-canvas repaint is allowed). Assert zero global layout calls per move. Run existing graph package tests to protect non-Constellation callers.

### P08 — Placement orchestration and live reconciliation

**Files:** new `F/domain/use_case/constellation_anchor_case.dart`; `F/domain/use_case/constellation_field_case.dart`, `F/ui/bloc/constellation_state.dart`, `constellation_cubit.dart`; `C/domain/entity/realtime/realtime_entity_change.dart`, `docs/contracts/realtime-entity-contract.json`, server/client realtime architecture tests and the client impact-subscriber map specified in C8. Reuse `RealtimeSyncPort` without changing its generic interface.
...
**Files:** new `packages/client/integration_test/constellation_pinning_test.dart` and `packages/client/test_driver/constellation_pinning_multiclient_web_test.dart`; extend `scripts/run_realtime_multiclient_web_local.sh`; reuse session/login/checkpoint patterns from `packages/client/test_driver/realtime_multiclient_web_test.dart` (extract shared helpers only as needed); TestIds and journal.

Use real UI for drag/drop/pin, camera and filters. QA APIs may prepare users/topology/Requests and read server truth; they cannot substitute for the tested interaction. The single-app Flutter runner covers the first test only. For multi-client acceptance reuse the existing dedicated WebDriver infrastructure: two independent authenticated browser sessions of the same account and a third unauthorized account, with fresh real UI actions in the first and live convergence in the second (no manual refresh).

Extend the dedicated runner with `REALTIME_MULTICLIENT_DRIVER`, restricted to the existing `realtime_multiclient_web_test.dart` (default, behavior preserved) and new `constellation_pinning_multiclient_web_test.dart`; pass the selected driver to both invocation sites. Add `REALTIME_MULTICLIENT_ACTOR_ECHO_ENABLED` (default true) to the server-start environment so the new proof can run with false. The new driver must cover live and reconnect convergence, record per-journey evidence, and use the existing private login/session infrastructure. API-origin “remote device” mutations may additionally inject deterministic races, but do not count them as two-browser interaction acceptance. Assert persisted normalized coordinates, not screenshot pixel equality.
...
REALTIME_MULTICLIENT_DRIVER=constellation_pinning_multiclient_web_test.dart REALTIME_MULTICLIENT_RUNS=1 REALTIME_MULTICLIENT_NEGATIVE_PROOFS=false REALTIME_MULTICLIENT_ACTOR_ECHO_ENABLED=false ./scripts/run_realtime_multiclient_web_local.sh
```
...
Run the single-app and dedicated multi-client runners sequentially. Before browser execution check port 8888; do not start a duplicate Flutter server. The dedicated runner requires port 2080 free and owns its server/Flutter/Chrome sessions; stop only task-owned processes before invoking it and report a pre-existing unrelated process as a blocked prerequisite. Do not treat API-emulated device actions as a replacement for this runner. Follow `docs/local-integration-tests.md` for the stack. Record separately tests passed, infrastructure failures and unrun gates.
