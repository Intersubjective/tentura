...
- **UI:** Compact forward screen (`ForwardTopBar` with full-screen `ForwardSearchOverlay`, `CompactBeaconContextStrip`, `ForwardScopeLinks`, `ForwardRecipientRow` with inline personalized-note toggle + field, `ForwardBottomComposer` with collapsible shared note and forward CTA).
